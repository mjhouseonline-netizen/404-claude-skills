...
description: |
source_group: skills
imported_from: azure-identity-rust.md
...
1. **Use `DeveloperToolsCredential` for local dev** Ã¢â‚¬â€ automatically picks up Azure CLI
2. **Use `ManagedIdentityCredential` in production** Ã¢â‚¬â€ no secrets to manage
3. **Clone credentials** Ã¢â‚¬â€ credentials are `Arc`-wrapped and cheap to clone
4. **Reuse credential instances** Ã¢â‚¬â€ same credential can be used with multiple clients
5. **Use `tokio` feature** Ã¢â‚¬â€ `cargo add azure_identity --features tokio`
