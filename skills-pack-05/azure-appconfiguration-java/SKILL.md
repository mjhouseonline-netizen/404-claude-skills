...
description: |
source_group: skills
imported_from: azure-appconfiguration-java.md
...
1. **Use labels** Ã¢â‚¬â€ Separate configurations by environment (Dev, Staging, Production)
2. **Use snapshots** Ã¢â‚¬â€ Create immutable snapshots for releases
3. **Feature flags** Ã¢â‚¬â€ Use for gradual rollouts and A/B testing
4. **Secret references** Ã¢â‚¬â€ Store sensitive values in Key Vault
5. **Conditional requests** Ã¢â‚¬â€ Use ETags for optimistic concurrency
6. **Read-only protection** Ã¢â‚¬â€ Lock critical production settings
7. **Use Entra ID** Ã¢â‚¬â€ Preferred over connection strings
8. **Async client** Ã¢â‚¬â€ Use for high-throughput scenarios
