...
description: |
source_group: skills
imported_from: azure-ai-voicelive-dotnet.md
...
**Required Role**: `Cognitive Services User` (assign in Azure Portal Ã¢â€ â€™ Access control)
...
```
VoiceLiveClient
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ VoiceLiveSession (WebSocket connection)
    Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ ConfigureSessionAsync()
    Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ GetUpdatesAsync() Ã¢â€ â€™ SessionUpdate events
    Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ AddItemAsync() Ã¢â€ â€™ UserMessageItem, FunctionCallOutputItem
    Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ SendAudioAsync()
    Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ StartResponseAsync()
```
...
if (serverEvent is SessionUpdateResponseFunctionCallArgumentsDone functionCall)
{
    if (functionCall.Name == "get_current_weather")
    {
        ...
        string weatherInfo = $"The weather in {location} is sunny, 75Ã‚Â°F.";
...
1. **Always set both modalities** Ã¢â‚¬â€ Include `Text` and `Audio` for voice assistants
2. **Use `AzureSemanticVadTurnDetection`** Ã¢â‚¬â€ Provides natural conversation flow
3. **Configure appropriate silence duration** Ã¢â‚¬â€ 500ms typical to avoid premature cutoffs
4. **Use `using` statement** Ã¢â‚¬â€ Ensures proper session disposal
5. **Handle all event types** Ã¢â‚¬â€ Check for errors, audio, text, and function calls
6. **Use DefaultAzureCredential** Ã¢â‚¬â€ Never hardcode API keys
