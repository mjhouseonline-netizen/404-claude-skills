...
description: |
source_group: skills
imported_from: azure-compute-batch-java.md
...
1. **Use Entra ID** Ã¢â‚¬â€ Preferred over shared key for authentication
2. **Use management SDK for pools** Ã¢â‚¬â€ `azure-resourcemanager-batch` supports managed identities
3. **Batch task creation** Ã¢â‚¬â€ Use `createTaskCollection` or `createTasks` for multiple tasks
4. **Handle LRO properly** Ã¢â‚¬â€ Pool resize, delete operations are long-running
5. **Monitor task counts** Ã¢â‚¬â€ Use `getJobTaskCounts` to track progress
6. **Set constraints** Ã¢â‚¬â€ Configure `maxWallClockTime` and `maxTaskRetryCount`
7. **Use low-priority nodes** Ã¢â‚¬â€ Cost savings for fault-tolerant workloads
8. **Enable autoscale** Ã¢â‚¬â€ Dynamically adjust pool size based on workload
