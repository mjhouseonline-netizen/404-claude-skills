...
description: |
source_group: skills
imported_from: azure-eventhub-dotnet.md
...
1. **Use `EventProcessorClient` for receiving** Ã¢â‚¬â€ Never use `EventHubConsumerClient` in production
2. **Checkpoint strategically** Ã¢â‚¬â€ After N events or time interval, not every event
3. **Use partition keys** Ã¢â‚¬â€ For ordering guarantees within a partition
4. **Reuse clients** Ã¢â‚¬â€ Create once, use as singleton (thread-safe)
5. **Use `await using`** Ã¢â‚¬â€ Ensures proper disposal
6. **Handle `ProcessErrorAsync`** Ã¢â‚¬â€ Always register error handler
7. **Batch events** Ã¢â‚¬â€ Use `CreateBatchAsync()` to respect size limits
8. **Use buffered producer** Ã¢â‚¬â€ For high-volume scenarios with automatic batching
