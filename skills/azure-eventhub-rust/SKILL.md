...
description: |
source_group: skills
imported_from: azure-eventhub-rust.md
...
Client library for Azure Event Hubs Ã¢â‚¬â€ big data streaming platform and event ingestion service.
...
- **Namespace** Ã¢â‚¬â€ container for Event Hubs
- **Event Hub** Ã¢â‚¬â€ stream of events partitioned for parallel processing
- **Partition** Ã¢â‚¬â€ ordered sequence of events
- **Producer** Ã¢â‚¬â€ sends events to Event Hub
- **Consumer** Ã¢â‚¬â€ receives events from partitions
...
1. **Reuse clients** Ã¢â‚¬â€ create once, send many events
2. **Use batches** Ã¢â‚¬â€ more efficient than individual sends
3. **Check batch capacity** Ã¢â‚¬â€ `try_add_event_data` returns false when full
4. **Process partitions in parallel** Ã¢â‚¬â€ each partition can be consumed independently
5. **Use consumer groups** Ã¢â‚¬â€ isolate different consuming applications
6. **Handle checkpointing** Ã¢â‚¬â€ use `azure_messaging_eventhubs_checkpointstore_blob` for distributed consumers
