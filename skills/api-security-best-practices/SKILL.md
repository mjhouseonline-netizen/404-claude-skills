...
source_group: skills
imported_from: api-security-best-practices.md
---
...
- Ã¢Å“â€¦ Use strong JWT secrets (256-bit minimum)
- Ã¢Å“â€¦ Set short expiration times (1 hour for access tokens)
- Ã¢Å“â€¦ Implement refresh tokens for long-lived sessions
- Ã¢Å“â€¦ Store refresh tokens in database (can be revoked)
- Ã¢Å“â€¦ Use HTTPS only
- Ã¢Å“â€¦ Don't store sensitive data in JWT payload
- Ã¢Å“â€¦ Validate token issuer and audience
- Ã¢Å“â€¦ Implement token blacklisting for logout
```
...
**Solution:**
\`\`\`javascript
// Ã¢ÂÅ’ Bad: Only checks authentication
app.delete('/api/posts/:id', authenticateToken, async (req, res) => {
  await prisma.post.delete({ where: { id: req.params.id } });
  res.json({ success: true });
});

// Ã¢Å“â€¦ Good: Checks both authentication and authorization
app.delete('/api/posts/:id', authenticateToken, async (req, res) => {
...
**Solution:**
\`\`\`javascript
// Ã¢ÂÅ’ Bad: Exposes database details
...
// Ã¢Å“â€¦ Good: Generic error message
app.post('/api/users', async (req, res) => {
  try {
    const user = await prisma.user.create({ data: req.body });
    res.json(user);
