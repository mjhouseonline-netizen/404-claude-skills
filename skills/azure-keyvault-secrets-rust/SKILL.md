...
description: |
source_group: skills
imported_from: azure-keyvault-secrets-rust.md
...
Client library for Azure Key Vault Secrets Ã¢â‚¬â€ secure storage for passwords, API keys, and other secrets.
...
1. **Use Entra ID auth** Ã¢â‚¬â€ `DeveloperToolsCredential` for dev, `ManagedIdentityCredential` for production
2. **Use `into_model()?`** Ã¢â‚¬â€ to deserialize responses
3. **Use `ResourceExt` trait** Ã¢â‚¬â€ for extracting names from IDs
4. **Handle soft delete** Ã¢â‚¬â€ deleted secrets can be recovered within retention period
5. **Set content type** Ã¢â‚¬â€ helps identify secret format
6. **Use tags** Ã¢â‚¬â€ for organizing and filtering secrets
7. **Version secrets** Ã¢â‚¬â€ new values create new versions automatically

## RBAC Permissions

Assign these Key Vault roles:
- `Key Vault Secrets User` Ã¢â‚¬â€ get and list
- `Key Vault Secrets Officer` Ã¢â‚¬â€ full CRUD
