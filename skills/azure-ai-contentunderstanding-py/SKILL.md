...
description: |
source_group: skills
imported_from: azure-ai-contentunderstanding-py.md
...
1. **Begin Analysis** Ã¢â‚¬â€ Start the analysis operation with `begin_analyze()` (returns a poller)
2. **Poll for Results** Ã¢â‚¬â€ Poll until analysis completes (SDK handles this with `.result()`)
3. **Process Results** Ã¢â‚¬â€ Extract structured results from `AnalyzeResult.contents`
...
1. **Use `begin_analyze` with `AnalyzeInput`** Ã¢â‚¬â€ this is the correct method signature
2. **Access results via `result.contents[0]`** Ã¢â‚¬â€ results are returned as a list
...
6. **Handle long-running operations** Ã¢â‚¬â€ video/audio analysis can take minutes
