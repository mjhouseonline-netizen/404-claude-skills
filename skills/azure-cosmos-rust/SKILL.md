...
description: |
source_group: skills
imported_from: azure-cosmos-rust.md
...
Client library for Azure Cosmos DB NoSQL API Ã¢â‚¬â€ globally distributed, multi-model database.
...
1. **Always specify partition key** Ã¢â‚¬â€ required for point reads and writes
2. **Use `into_model()?`** Ã¢â‚¬â€ to deserialize responses into your types
3. **Derive `Serialize` and `Deserialize`** Ã¢â‚¬â€ for all document types
4. **Use Entra ID auth** Ã¢â‚¬â€ prefer `DeveloperToolsCredential` over key auth
5. **Reuse client instances** Ã¢â‚¬â€ clients are thread-safe and reusable
