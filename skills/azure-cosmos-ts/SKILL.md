...
description: |
source_group: skills
imported_from: azure-cosmos-ts.md
...
Data plane SDK for Azure Cosmos DB NoSQL API operations Ã¢â‚¬â€ CRUD on documents, queries, bulk operations.

> **Ã¢Å¡Â Ã¯Â¸Â Data vs Management Plane**
...
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ Database
    Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ Container
        Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Items (documents)
        Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ Scripts (stored procedures, triggers, UDFs)
        Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ Conflicts
```
...
1. **Use AAD authentication** Ã¢â‚¬â€ Prefer `DefaultAzureCredential` over keys
2. **Always use parameterized queries** Ã¢â‚¬â€ Prevents injection, improves plan caching
3. **Specify partition key** Ã¢â‚¬â€ Avoid cross-partition queries when possible
4. **Use bulk operations** Ã¢â‚¬â€ For multiple writes, use `executeBulkOperations`
5. **Handle 429 errors** Ã¢â‚¬â€ Implement retry logic with exponential backoff
6. **Use ETags for concurrency** Ã¢â‚¬â€ Prevent lost updates in concurrent scenarios
7. **Close client on shutdown** Ã¢â‚¬â€ Call `client.dispose()` in cleanup
