...
description: |
source_group: skills
imported_from: azure-monitor-ingestion-java.md
...
1. **Batch logs** Ã¢â‚¬â€ Upload in batches rather than one at a time
2. **Use concurrency** Ã¢â‚¬â€ Set `maxConcurrency` for large uploads
3. **Handle partial failures** Ã¢â‚¬â€ Use error consumer to log failed entries
4. **Match DCR schema** Ã¢â‚¬â€ Log entry fields must match DCR transformation expectations
5. **Include TimeGenerated** Ã¢â‚¬â€ Most tables require a timestamp field
6. **Reuse client** Ã¢â‚¬â€ Create once, reuse throughout application
7. **Use async for high throughput** Ã¢â‚¬â€ `LogsIngestionAsyncClient` for reactive patterns
