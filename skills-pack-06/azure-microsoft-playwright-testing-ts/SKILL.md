...
source_group: skills
imported_from: azure-microsoft-playwright-testing-ts.md
...
1. **Use Entra ID auth** Ã¢â‚¬â€ More secure than access tokens
2. **Provide explicit credential** Ã¢â‚¬â€ Always pass `credential: new DefaultAzureCredential()`
3. **Enable artifacts** Ã¢â‚¬â€ Set `trace: "on-first-retry"`, `video: "retain-on-failure"` in config
4. **Scale workers** Ã¢â‚¬â€ Use `--workers=20` or higher for parallel execution
5. **Region selection** Ã¢â‚¬â€ Choose region closest to your test targets
6. **HTML reporter first** Ã¢â‚¬â€ When using Azure reporter, list HTML reporter before Azure reporter
