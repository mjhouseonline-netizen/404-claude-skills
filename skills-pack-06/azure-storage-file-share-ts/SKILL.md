...
description: |
source_group: skills
imported_from: azure-storage-file-share-ts.md
...
SDK for Azure File Share operations Ã¢â‚¬â€ SMB file shares, directories, and file operations.
...
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ ShareClient (share level)
    Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ ShareDirectoryClient (directory level)
        Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ ShareFileClient (file level)
```
...
1. **Use connection strings for simplicity** Ã¢â‚¬â€ Easiest setup for development
2. **Use DefaultAzureCredential for production** Ã¢â‚¬â€ Enable managed identity in Azure
3. **Set quotas on shares** Ã¢â‚¬â€ Prevent unexpected storage costs
4. **Use streaming for large files** Ã¢â‚¬â€ `uploadStream`/`downloadToFile` for files > 256MB
5. **Use ranges for partial updates** Ã¢â‚¬â€ More efficient than full file replacement
6. **Create snapshots before major changes** Ã¢â‚¬â€ Point-in-time recovery
7. **Handle errors gracefully** Ã¢â‚¬â€ Check `RestError.statusCode` for specific handling
8. **Use `*IfExists` methods** Ã¢â‚¬â€ For idempotent operations
