...
description: |
source_group: skills
imported_from: azure-mgmt-fabric-dotnet.md
...
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ SubscriptionResource
    Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ ResourceGroupResource
        Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬ FabricCapacityResource
```
...
3. **Always use `DefaultAzureCredential`** Ã¢â‚¬â€ never hardcode credentials
4. **Handle `RequestFailedException`** for ARM API errors
5. **Use `CreateOrUpdateAsync`** for idempotent operations
6. **Suspend when not in use** Ã¢â‚¬â€ Fabric capacities bill for compute even when idle
7. **Check provisioning state** before performing operations on a capacity
8. **Use appropriate SKU** Ã¢â‚¬â€ Start small (F2/F4) for dev/test, scale up for production
...
1. **Capacity names must be globally unique** Ã¢â‚¬â€ Fabric capacity names must be unique across all Azure subscriptions
2. **Suspend doesn't delete** Ã¢â‚¬â€ Suspended capacities still exist but don't bill for compute
3. **SKU changes may require downtime** Ã¢â‚¬â€ Scaling operations can take several minutes
4. **Admin UPNs must be valid** Ã¢â‚¬â€ Capacity administrators must be valid Azure AD users
5. **Location constraints** Ã¢â‚¬â€ Not all SKUs are available in all regions; use `GetSkusFabricCapacitiesAsync` to check
6. **Long provisioning times** Ã¢â‚¬â€ Capacity creation can take 5-15 minutes
