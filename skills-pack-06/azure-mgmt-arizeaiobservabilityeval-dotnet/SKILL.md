...
description: |
source_group: skills
imported_from: azure-mgmt-arizeaiobservabilityeval-dotnet.md
...
1. **Use async methods** Ã¢â‚¬â€ All operations support async/await
2. **Handle long-running operations** Ã¢â‚¬â€ Use `WaitUntil.Completed` or poll manually
3. **Use GetIfExistsAsync** Ã¢â‚¬â€ Avoid exceptions for conditional logic
4. **Implement retry policies** Ã¢â‚¬â€ Configure via `ArmClientOptions`
5. **Use resource identifiers** Ã¢â‚¬â€ For direct resource access without listing
6. **Close clients properly** Ã¢â‚¬â€ Use `using` statements or dispose explicitly
