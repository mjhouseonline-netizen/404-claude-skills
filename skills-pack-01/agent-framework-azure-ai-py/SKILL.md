...
source_group: skills
imported_from: agent-framework-azure-ai-py.md
...
```
User Query Ã¢â€ â€™ AzureAIAgentsProvider Ã¢â€ â€™ Azure AI Agent Service (Persistent)
                    Ã¢â€ â€œ
...
def get_weather(
    ...
    return f"Weather in {location}: 72Ã‚Â°F, sunny"
...
async def main():
    async with (
        ...
        print(f"{weather.location}: {weather.temperature}Ã‚Â°{weather.unit}")
```
